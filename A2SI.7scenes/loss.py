...
class FineMatchingLoss(nn.Module):
    ...
    def forward(self, data_dict, output_dict):
        ...
        img_corr_pixels = img_corr_pixels.long()  # (M, 2)
...
#########################################################################################
        ...
        # pcd_corr_feats = output_dict["pcd_corr_feats"]
        # img_corr_feats = output_dict["img_corr_feats"]
        # img_corr_feats_norm = F.normalize(img_corr_feats, p=2, dim=1)  # N x C
        # pcd_corr_feats_norm = F.normalize(pcd_corr_feats, p=2, dim=1)

        # dist_img = img_corr_feats_norm @ img_corr_feats_norm.T  # (N x N)
        # dist_pcd = pcd_corr_feats_norm @ pcd_corr_feats_norm.T

        # loss_sim = F.mse_loss(dist_img, dist_pcd)
...
class FeatureConsistencyLoss(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.start_epoch = getattr(cfg.loss, "start_epoch", 10)
        self.max_weight = getattr(cfg.loss, "feature_consistency_weight", 0.1)
        self.grow_epochs = getattr(cfg.loss, "feature_consistency_warmup_epochs", 20)  # 从start到start+grow慢慢增大

    def forward(self, output_dict, current_epoch):
        if "img_corr_feats" not in output_dict or "pcd_corr_feats" not in output_dict:
            return torch.tensor(0.0, device=output_dict["img_feats_c"].device)
        ...
        img_corr_feats = F.normalize(img_corr_feats, p=2, dim=1)
        pcd_corr_feats = F.normalize(pcd_corr_feats, p=2, dim=1)

        dist_img = img_corr_feats @ img_corr_feats.T
        dist_pcd = pcd_corr_feats @ pcd_corr_feats.T

        loss_sim = 1.5*F.mse_loss(dist_img, dist_pcd)

        # 权重从0逐渐增加到max_weight
        ramp_epoch = current_epoch - self.start_epoch
        ramp_ratio = min(ramp_epoch / max(self.grow_epochs, 1), 1.0)
        dynamic_weight = self.max_weight * ramp_ratio

        return dynamic_weight * loss_sim
...
class OverallLoss(nn.Module):
    ...
    def forward(self, data_dict, output_dict, epoch):
